    ...
    var body: some View {
        NavigationStack {
            List {
                Section {
                    HStack(spacing: 14) {
                        ...
                        //
                        // **素材は `AppIcon` と同一のPNGです（2026-08-08）。**
                        // アセットカタログのアイコンは `Image(_:)` で直接引けないため、
                        // 同じファイルをここへ複製しています。
                        // **アイコンを差し替えるときは両方を更新すること。**
                        // 片方だけ替えて、設定画面に旧アイコンが残ったままビルド11を出荷した前例があります。
            ...
            .alert("テーマを既定に戻しますか？", isPresented: $showsThemeResetConfirmation) {
                ...
                // **残っている設定だけを挙げます。** カードの色を選ぶ設定は
                // 2026-08-06に削除しており、案内に残すと戻る対象を誤解させます。
                Text("ボタンの色とグラフの表示が、既定に戻ります。")
            }
