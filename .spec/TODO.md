...
- [ ] `CostType` に色を持たせる（`title` / `systemImage` と同じ流儀で型側に置く）
- [ ] **先にテストを書く**：`ReportEntry` に `costType` を持たせ、`ReportCalculator` が詰めること
- [ ] `ReportEntry` へ `costType` を追加する
- [ ] **先にテストを書く**：種別ごとの集計（金額の合計、見込みが1件でもあれば見込み扱い、
      0円の種別を出さない、0件・1件、全件同額）
- [ ] 種別ごとの集計を実装する
- [ ] **先にテストを書く**：バブルの配置（面積が金額に比例する、重ならない、
      初期倍率で全件が収まる）
- [ ] バブルの配置計算を実装する（ビューから切り離した純粋な計算にする）
